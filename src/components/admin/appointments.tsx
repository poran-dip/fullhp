...
const AdminAppointments: React.FC = () => {
  ...
  const [formDoctorId, setFormDoctorId] = useState<string | undefined>(
    undefined,
  );
  ...
  const getFilteredDoctors = () => {
    if (
      !formSpecialization ||
      formSpecialization === "" ||
      formSpecialization === "none"
    ) {
      return doctors;
    }
    return doctors.filter(
      (doctor) => doctor.specialization === formSpecialization,
    );
  };
  ...
  const handleEditAppointment = (appointment: Appointment) => {
    ...
    setFormDate(
      appointment.dateTime ? new Date(appointment.dateTime) : undefined,
    );
    setFormTime(
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setAppointments((prev) =>
        prev.filter((a) => a.id !== currentAppointment.id),
      );
                  ...
                  onValueChange={(value) => {
                    ...
                    if (formDoctorId) {
                      const currentDoctor = doctors.find(
                        (doc) => doc.id === formDoctorId,
                      );
                      if (
                        !currentDoctor ||
                        currentDoctor.specialization !== value
                      ) {
                        setFormDoctorId(undefined);
                      }
                    }
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <div className="col-span-3">
                <Select
                  value={formStatus}
                  onValueChange={(
                    value: "NEW" | "PENDING" | "COMPLETED" | "CANCELED",
                  ) => setFormStatus(value)}
                >
                  <SelectTrigger>
